...
int read_png(const char* filename, jpeg_data* imageData) {
	printf("Reading PNG...\n");
    ...
    // imagData->channels = (int) png_get_channels(png_ptr, info_ptr);
	imageData->num_pixel = imageData->width * imageData->height;

	if (alloc_jpeg_data(imageData))
		return -1;
	...
	printf("Reading PNG Almost End\n");
	for (int i=0; i<imageData->height; ++i) {
		for (int j=0; j<imageData->width; ++j) {
			imageData->red[i*imageData->width + j] = imageData->data[(i*imageData->width + j)*3 + 0];
			imageData->green[i*imageData->width + j] = imageData->data[(i*imageData->width + j)*3 + 1];
			imageData->blue[i*imageData->width + j] = imageData->data[(i*imageData->width + j)*3 + 2];
		}
	}

	printf("Reading PNG End\n");
...
void dct(int blocks_horiz, int blocks_vert, int in[], double out[])
{
	...
	for (v=0; v<blocks_vert; v++)
		...
		for (h=0; h<blocks_horiz; h++)
			...
			// blocks_horiz * 8 (width) * 8 (go down 8 rows one time)
}
...
int main(int argc, char** args)
{
	...
	fflush(stdout);
	write_file("images/out.jpg", &data);
	printf("%10.3f ms\n", timer());
